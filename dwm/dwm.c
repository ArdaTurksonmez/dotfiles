...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (TEXTW(m->sel->name) > w) /* when the title is bigger than the width of the title rectangle, don't center */
				drw_text(drw, x, 0, w - 2 * sp, bh, lrpad / 2, m->sel->name, 0);
			else /* center window title */
				drw_text(drw, x, 0, w - 2 * sp, bh, MAX((m->ww - ((int)TEXTW(m->sel->name) - lrpad)) / 2 - x, lrpad / 2), m->sel->name, 0);
			if (m->sel->isfloating)
